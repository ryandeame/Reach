(() => {
  ...
  const showDesktopRequiredNotification = () => {
    showNotification("Switch to desktop mode to run get-person-info.");
  };

  const isContactInfoOverlay = () =>
    window.location.pathname.endsWith("/overlay/contact-info/");

  const abortForWrongPage = () => {
    const message =
      'Go to a URL that ends in "/overlay/contact-info/" to run get-person-info.';
  ...
  if (isMobileMode()) {
    showDesktopRequiredNotification();
    return;
  }
  ...
  const personInfo = compactObject({
    name: getName(),
